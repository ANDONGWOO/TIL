...
def dfs(v):
    ...
    for z in graph[v]:#graph[1~n까지][1, 2], [1], [2, 3], [3, 4, 5], [1]
        ...
        if not matching_list[z] or dfs(matching_list[z]):#3/1
            # print(not matching_list[z] or dfs(matching_list[z]))#[False, False, True, False, False, True]
            ...
            #print(v)#1,1,2,3,4,4/실행하고 매칭x?
            matching_list[z] = v#1,1,2,3,4,4/z는 1부터 시작
            # print(matching_list[z])
            # print(matching_list)#그림순서
            return True
...
# print([1 for i in matching_list if i > 0])#[1, 1, 1, 1]#매칭 이면 1
